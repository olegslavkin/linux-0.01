# Введение
Данное руководство описывает попытку повторить, на сколько это возможно, действия Линуса Торвальдса по компиляции и запуску самой первой версии ядра Linux 0.01. Причины побудивщие Линуса начать разработка ядра хорошо написаны в книге "[Ради удовольствия: Рассказ нечаянного революционера](https://ru.wikipedia.org/wiki/%D0%A0%D0%B0%D0%B4%D0%B8_%D1%83%D0%B4%D0%BE%D0%B2%D0%BE%D0%BB%D1%8C%D1%81%D1%82%D0%B2%D0%B8%D1%8F)" (далее J4F) и во множестве других источниках. Но цель данного руководства описать пошаговый и максимально детальный процесс компиляции и запуска ядра Linux.

Сам процесс сборки ядра Linux, по unix'овому очень прост, достаточно выполнить команду `make` и на выходе получить готовый бинарный файл, который надо записать на дискету и загрузится с неё. Но чтобы этот процесс сборки прошел успешно надо сделать большую подготовку. Вот этим мы и займемся в данном руководстве.

В начале создадим пустую виртуальную машину в эмуляторе [86Box](https://86box.readthedocs.io/) с характеристиками близкими к компьютеру Линуса в то время, установим оригинальную версию операционной системы Minix 1.5.10, применим патчи "царя и бога Minix-386" (c) Брюса Эванса, поставим порт компилятора gcc версии 1.37.1 для Minix-386 от [Алана Блэка](https://www.cs.cmu.edu/~awb/) (Alan W Black) и [Ричарда Тобина](https://www.cogsci.ed.ac.uk/~richard/) (Richard Tobin) и в самом конце соберем и запустим непосредственно ядро Linux с bash'ем внутри.

После каждого этапа, я сохранял состояние виртуальной машины, жесткого диска, а также сохранялись образы дискет, которые подвергались изменению. Мы можете пройти все шаги самостоятельно или распаковать любой из архивов и продолжить выполнение инструкции.

Все дистрибутивы, скриншоты, конфигурации, эту инструкцию, архивы с резервными копиями по каждому этапу и даже бинарный AppImage 86box (для Linux x86_64), можно найти в репозитории https://github.com/olegslavkin/linux-0.01.
...
- [Установка оригинальной 16-битной версии Minix 1.5.10](#установка-оригинальной-16-битной-версии-minix-1510)
  ...
  - [Установка (копирование) Minix 1.5.10 на жесткий диск](#установка-копирование-minix-1510-на-жесткий-диск)
  - [Тестирование работоспособности Minix](#тестирование-работоспособности-minix)
- [Установка патчей от Брюса Аванса (Bruce Evans' 386 patches) ака Minix-386 ](#установка-патчей-от-брюса-аванса-bruce-evans-386-patches-ака-minix-386-)
...
Прежде всего чем начать процесс сборки и компиляции Linux, хочется определится с условиями с которыми приходилось работать Линусу Торвальдсу во время работы над ядром Linux. В первую очередь интересует, на каком персональном компьютере он работал тогда и какие были у него характеристики. Да, мы достоверно знаем на каком процессоре он работал, и ещё некоторые характеристики, но я нигде в интернете не встречал (или плохо искал?) полную спецификацию его ПК и я решил, на основании различных открытых источников в сети, произвести "исследование" по этому вопросу. Вот на каком "железе", **по моему мнению**, работал автор ядра Linux.

Буквально в следующем разделе этого руководства, *приблизительно* сконфигурируем виртуальный компьютер в эмуляторе 86Box.
...
| FDD A                   | 3.5 1.44M <sup>[8](#note-35fdd)</sup> |
| FDD B                   | 5.25 1.2M <sup>[9](#note-dualfdd)</sup> |
...
- [6] В J4F описывалось, что было 4 Мб (скорее всего такой объем был изначально), но книге Линус пишет: "Помню, мне пришлось выйти из дома, чтобы увеличить ОЗУ с 4 до 8 мегабайт" и непонятно осталась эта дополнительная память в его ПК или он взял её у кого-то временно. Я думаю, что осталась, т.к. в ядре 0.01 в нескольких местах указано, что уже 8Мб. <a name="note-ram"></a>
- [7] [Возможно](https://usenetarchives.com/view.php?id=comp.sys.ibm.pc.hardware&mid=PDEwNDE0QGh5ZHJhLkhlbHNpbmtpLkZJPg) в начале был видеоадаптер Trident TVGA 8800 (?). <a name="note-video"></a>
- [8] В руководстве указано устройство `/dev/PS0` (это 3.5" 1.44M). <a name="note-35fdd"></a>
- [9] Предположительно, что дисковод был, т.к. в то время было распространенной практикой иметь дисководы и на 3.5, и на 5.25. Косвенное доказательно наличие дисковода на 5.25 это то, что в J4F говорится, что во время установки Minix Линусу потребовалось вставить 16 дискет (*скорее всего, на самом деле вставить только 15. Первые 3 дискеты являются загрузочными и он использовал одну из них*) и несмотря на то, что Minix-1.5 от PH на дискетах 3.5 официально [издавался](http://opennet.ru/docs/FAQ/OS/minix-info.html) у меня есть сомнения, что PH оставили дистрибутив на 17 3.5 дискетах. [Японское](https://habrastorage.org/webt/3e/wd/p5/3ewdp5n7zc0onpcjmpe_rri5cjq.jpeg) издание книги уместились на 6. <a name="note-dualfdd"></a>
...
Данная инструкция написана и тестировалась на HP ProBook 440 G6 с операционной системой `Ubuntu 20.04.6 LTS` и версией эмулятора `86Box-Linux-x86_64-b4724.AppImage` (AppImage есть в `bin` директории репозитория).
...
Далее необходимо распаковать архив с пустым и неразмеченным виртуальным жестким диском на 68Мб.
...
> Внимательные читатели спросят, почему диск 1, а не 2 как в таблице выше? И почему он совершенно с другими характеристиками? Отвечу, изначально когда начал писать данное руководство, о 2-х жестких дисках и их характеристиках ещё не знал и поэтому создал только 1 диск. Я попытался адаптировать руководство под 2 диска, но не справился с разметкой в Minix :(
> 
> В будущих релизах, адаптирую под 2 жестких диска и, возможно, не буду хранить вместе и 16 и 32 битные утилиты Minix/Minix-386.


И затем запустить эмулятор в режиме конфигурацию виртуального PC. 

> Если у вас до этого не был установлен 86box, то также необходимо будет установить набор [ROM](https://github.com/86Box/roms/releases/latest) образов. [Подробнее см. в документации](https://86box.readthedocs.io/en/latest/usage/gettingstarted.html).

```
$ ./86box -S
```
И на основании скриншотов ниже конфигурируем виртуальный компьютер, с которым будем работать на протяжении всего этого руководства.

<spoiler title="Конфигурация виртуального компьютера в 86box.">

В качестве "Machine", возможно лучше было бы использовать "[SiS 310] ASUS ISA-386C", но как писал выше, у меня нет данных какая материнская плата была изначально.
...
Приступим к первому этапу, установим оригинальную версию Minix 1.5.10 c оригинальных образов дискет.

Содержимое данного раздела основано на 2-ой главе оригинального [MINIX 1.5 REFERENCE MANUAL](https://github.com/olegslavkin/linux-0.01/blob/master/manual/minix-15-reference-manual-1991.pdf).

> Иногда в сети можно встретить название "Minix 1.5.10 PH", ещё встречал "Minix PH". PH - это Prentice-Hall, название издательства, которое и издавало книгу [Andrew S. Tanenbaum - Operating Systems Design and Implementation-Prentice-Hall (1987)](https://github.com/olegslavkin/linux-0.01/blob/master/manual/Andrew%20S.%20Tanenbaum%20-%20Operating%20Systems%20Design%20and%20Implementation-Prentice-Hall%20(1987).pdf). Эти те самые "719 страниц в мягком красном переплете, можно сказать, поселились у меня в постели" (с) (J4F).
...
$ ./86box
```
Первым делом заходим в настройки BIOS (нажимаем клавишу `DEL`). Устанавливаем характеристики жесткого диска (наиболее подходящий нам тип `type 36`) и флоппи дисководов. ВАЖНО, что бы `Floppy drive A:` был 360K 5.25. Сохраняем настройки BIOS и выходим.
...
> Как видите из скриншота ниже, Minix корректно работает, если ввести 23 год, а вот недавно на linkmeup был опубликован забавный скриншот, в которой SunOS 4.1.4 писала [предупреждение](https://t.me/linkmeup_podcast/7372).

В Minix 1.5.10, имеется, по меньшей мере, 2 учетные записи. Первая из них - это привилегированная `root`овая учетная запись с паролем `Geheim`, а также имеется учетная запись обычного пользователя `ast` с паролем `Wachtwoord`. Авторизуйтесь как `root`.

![login](https://habrastorage.org/webt/3g/cm/e8/3gcme8tv_m-ph34-t8tbqbfz3di.png)

<spoiler title="Настраиваем (опционально) getty">

На этом этапе упростим дальнейшее взаимодействие с Minix. Вводить команды в окне эмулятора это с одной стороны более "правильный" путь, но всё же это эмулятор, плюс вводить команды таким способом это очень медленно, да и copy-paste с этой инструкции будет невозможен :)

Но, на наше счастье, Minix 1.5.10 поддерживает [getty](https://ru.wikipedia.org/wiki/Getty), а значит благодаря возможности 86Box пробрасывать виртуальный COM-порт как устройство псевдотерминала на хостовую операционную систему, можно с помощью minicom, (c)kermit и других подобных утилит подключится к Minix. 
...
Однако на этапе установки, с коробки, нет возможности использовать getty, но очень легко это исправить. Для этого смонтируйте диску disk04 в хостовой операционной системе (современный Linux всё ещё понимает старую файловую систему minix) и добавьте 1 строку в файл `etc/ttys`.
```
...
```
> Для данной инструкции строка уже добавлена в disk04, но если хотите сделать это самостоятельно в репозитории есть оригинальная дискета [disk04.img.orig](https://github.com/olegslavkin/linux-0.01/raw/master/dist/minix/disk04.img.orig)
...
<spoiler title="Подключится можно не только с хоста на Minix, но и в обратную сторону...">
...
> Также желании можно передавать и принимать файлы (*правда криво*) с помощью протокола zmodem (через `sz` и `rz`), но ИМХО проще смонтировать на хосте дискету или раздел жесткого диска и скопировать туда/оттуда нужные файлы.
...
n
```
В результате получится следующая таблица разделов.
```
                          ----first----  -----last----  --------sectors-------
...
```
Далее необходимо отформатировать все 3 minix партиции (раздела) жесткого диска файловой системой. Для этого, для каждой партиции, выполните команду `mkfs /dev/hdX <РАЗМЕР В БЛОКАХ>`, где `X` - это номер партиции, а `РАЗМЕР В БЛОКАХ` - это размер в партиции в блоках (по 1024 байта). Рассчитывается как размер секторов разделенный на 2.
```
# mkfs /dev/hd2 5100
# mkfs /dev/hd3 51340
# mkfs /dev/hd4 8024
...
Теперь вновь вставьте в виртуальный образ загрузочной дискеты `disk02` и перезагрузитесь.

После отображения загрузочного меню, необходимо изменить корневой раздел с которого будет производится загрузка. Во всех предыдущих шагах загружались с дискеты (`disk04`), но в этот раз необходимо загрузится с раздела жесткого диска. Для изменения корневого раздела в меню введите `r` (*select root device*), далее `h2`, затем <ENTER> и после этого вводим `=`.
...
> Если до этого подключилась к Minix с помощью minicom, то после загрузки с дискеты можно вновь будет к ней подключится. Если в эмуляторе 86box перегружаете виртуальный компьютер путем нажатия кнопки reset в меню эмулятора, то, как правило, устройство псевдотерминала остается тем же и в minicom ничего делать не надо, достаточно только ввести логин и пароль и далее продолжить следовать данному руководству.
...
```
Запуститься процесс инициализации раздела `/usr`. В процессе работы скрипта, скрипт попросит вставить по очереди дискеты от `disk05` по `disk17`.
...
Так как только что инициализировали раздел `/usr`, то нет больше необходимости вставлять дискету `disk05` в начале процесса загрузки операционной системы. Теперь можно автоматически монтировать партицию диска `hd3` как `/usr`. Для этого надо немного исправить `/etc/rc`.
```
# mined /etc/rc
```
> Если редактируете файлы в `minicom` и подобных утилитах, то, возможно, при скроллинге будут не корректно отображаться документ в терминале. Редактировать файл можно в основной консоле эмулятора, где после его сохранения можно продолжить вводить команды в minicom.
> 
> Или если Вы знаете как это исправить, напишите, пожалуйста мне, и я включу это в данную инструкцию.

Комментируем (или удаляем) строки монтирования дискеты (а также приглашение вставки дискеты) и добавляем запись монтирования раздела жесткого диска.
```
...
```
> В редакторе `mined` сохраняем изменений производится комбинаций `<CTRL+W>`, а выход из редактора `<CTRL+X>`
> 
...
Вводим команду `sync`, проверяем, что у нас вставлена загрузочная дискета `disk02` и вновь перезагружаемся.
...
Чтобы убедится, что операционная система установлена корректно, разработчиками были подготовлены тестовые программы (предварительно необходимо их скомпилировать) и sh скрипты. Для проведения тестирования необходимо перейти в каталог и откомпилируйте `*.c`. Рекомендуется компилировать как `root` (на последнем этапе устанавливаются для файлов права на выполнения)

```
# cd /usr/src/test
# make all
```
<spoiler title="Процесс компиляции тестовых программ, если хотите тест-кейсов">
...
В предыдущем разделе была установлена классическая 16-битная версия Minix 1.5.10, в том виде как изначально задумывалась автором, но цель этого руководства - это пройти шаги сделанные Линусом Торвальдсом по написанию собственной операционной системы Linux (*ну ладно, ладно не все шаги, сам Linux писать не будем :)*). Как упоминалось выше, Линус работал не в оригинальной, а в пропатченной версии Minux 1.5.10, также известный как Minix-386. Патчи были подготовлены Брюсом Эвансом, а Джон Налл (John Nall) написал [прекрасное руководство](https://github.com/olegslavkin/linux-0.01/raw/master/dist/minix-386/tutor.asc) по их применению.

Но прежде, чем мы начнем преображение, необходимо немного "проапгрейдить" наш виртуальный PC. Изначально, при установке классического Minix, были задействован дисковод 5.25 на 360Кб. Только установив данный тип виртуального дисковода в настройках эмулятора и BIOS, мне удалось произвести установку и написать это руководство. Но для загрузки 32-битной версии Minix понадобятся уже 5.25 1.2M дискеты (*а также позже и для Linux*), соответственно необходимо, убедиться, что в настройках эмулятора указан соответствующий тип дисковода, а в BIOS также установлен этот же тип.
...
> В сети есть [патч](https://usenetarchives.com/view.php?id=comp.os.minix&mid=PDc4MDZAbmlnZWwudWRlbC5FRFU%2B) позволяющий загрузится из различных типов дискет, но я не проверял его.
...
> Файлы содержащие на дискете были взяты из [архива](http://www.oldlinux.org/Linux.old/study/linux-travel/minix-386/zhao.rar) с сайта oldlinux.org. Кроме патчей там есть другие полезные файлы.
...
```
Создаем папку для компилятора bcc, распаковываем архивы с 16 и 32 битной версии компилятора bcc, и исходными кодами стандартной библиотеки Си.
```
cd /usr/oz
mkdir bcc
mv bcc*.tar bcc
...
> Использование пути `/local` обязательно. Если данный путь, вдруг, не подходит, необходимо будет его изменить `bcc.c` и перекомпилировать компилятор.
...
mkdir system
```
Производим компиляцию компонентов ядра Minix. Обратите внимание, что при редактирование makefile.cpp, необходимо чтобы все строки препроцессора cpp начинались с начала строки.
...
```
Компилируем `init`. В процессе линковки также могут появится предупреждения `warning: _exit (или _sbrk) redefined`, которые тоже игнорируем.
```
cd /usr/src/tools
...
Не обращяем внимание, что часть программ `readclock`, `date`, `wtmp`, `printroot` и `stty` задействованные иницилизирующем скриптом `/etc/rc` не смогли запуститься, они все ещё 16-ти битные. В следующем разделе исправим это.
...
После того, как проверили, что с дискеты успешно загрузились с новым ядром, необходимо произвести перекомпиляцию утилит. И для этого загружаемся в 16-битный minix (используем дискету `disk02` и раздел жесткого диска `h2`). И выполняем кросс-компиляцию 80386 версий бинарных утилит.
...
```
> Да, с точки зрения minix, это возможно не канон, но так можно одновременно иметь как 16-ти, так и 32-х битные версии программ. Также это упростит переключение между 16/32 режимами. Да, и просто я уже привык, что mount/umount находится в PATH.
...
/local/bin/bin3
```
Скрипт, который выполнили только что не переименовывает `/usr/binX` директорию. Сделайте это вручную.
```
...
Как известно ядро linux было скомпилировано с помощью gcc. И в оригинале это была собственная пропатченная версия gcc 1.40 от Линуса с поддержкой опции `-mstring-insns` (*+ может что-то ещё*). Где сейчас можно найти именно эту версию компилятора мне не известно. Возможно это [та самая версия](http://www.oldlinux.org/Linux.old/Linux-0.10/binaries/compilers/gccbin.tar.Z). В архиве файлы датированы 22 сентября 1991, очень-очень близкие к тем временам, с другой стороны эти даты нельзя ориентироваться. Возможно, это gcc для сборки в самом Linux, а не в Minix.

Я же пойду немного другим путем и буду использовать версию gcc-1.37.1 от Alan W Black и Richard Tobin, также известный как "awb-gcc from plains" иногда в сети её называли, просто "gcc from plains". В природе существует и [gcc-1.40](https://www.cs.cmu.edu/~awb/pub/minix/gasdiff.tar.gz) для Minix-386, но его не буду использовать, потому что а) патч не от Линуса Торвальдса от Энди Майкла (Andy Michael) б) есть только diff, надо его ещё и скомпилировать :) 

Загружаемся с дискеты `shoelace`, а затем меняем её на дискету c [gcc-1.37.1](https://github.com/olegslavkin/linux-0.01/raw/master/dist/gcc-1.37.1-plains/awb-gcc-1.37.1.img), монтируем ее и копируем 16-ти версию утилиты compress (*в minix использовалась [13-ти битная компресия](https://www.unix.com/man-page/minix/1/compress/)*).
```
mount /dev/at0 /user
...
: not found
```
Всё дело в том, что gcc генерирует бинарный файл напрямую несовместимый с minix. Есть два пути решения. Первый (популярный) - это сконвертировать в совместимый формат с помощью gcc2minix который входил в комплект gccbin.
```
...
Hello World
```
Второй вариант - это использовать патч [gnutoo](https://www.cs.cmu.edu/~awb/pub/minix/gnutoo.tar.gz), у меня он не заработал, но возможно я что-то сделал неправильно.
...
И вот, наконец-то, после всех этапов подготовки можно приступить непосредственно к компиляции ядра Linux, но для этого ещё чуть-чуть подготовится :). Помните, в самом начале, когда разбивали жесткий диск на разделы, у нас остался не использованный раздел `h4`, его оставили как раз для Linux.

Произведем форматирование раздела, а также подключим его в автомонтирование при старте Minix-386.
```
mkdir /linux
mkfs /dev/hd4 8024
mined /etc/rc
...
mkdir src
```
Вставляем дискету с исходными кодами Linux [linux-src-0.01](https://github.com/olegslavkin/linux-0.01/raw/master/dist/linux/src.img) и монтируем дискету. Сам архив c исходными кодами [linux-0.01.tar.Z](http://www.oldlinux.org/Linux.old/Linux-0.01/sources/system/linux-0.01.tar.Z) с сайте oldlinux.org. Примечательно, что на официальном сайте ядра Linux имеется [tar.gz версия](https://mirrors.edge.kernel.org/pub/linux/kernel/Historic/linux-0.01.tar.gz) от октября 1993 (*ядро 0.01, я напомню, ["родилось" 17.09.1991](https://lkml.org/lkml/2021/9/17/1018)*) это скорее всего "перепак" в формат архива поддерживаемый gnu утилитами того времени.
```
/bin/mount /dev/at0 /user
...
# Линуса был патч gnutoo (или ему подобный), поэтому конвертации не было
...
mined lib/Makefile
```
Адаптируем под наше "железо", используем дискеты на 5.25 1.2М вместо 3.5, задаем верхнею границу оперативной памяти, а также указываем, что будем использовать раздел `h4` жесткого диска и его характеристики.

> Вот на этапе и чуть ниже увидите почему, у меня есть сомнения, что у Линуса был компьютер с 4Мб оперативной памяти, скорее всего у него было 8Мб. Возможно, при покупке, было действительно 4, но потом был доукомплектован до 8Мб, как, возможно, собственно и 2-й жесткий диск.
...
```
Это происходит, как я понял, из-за малого размера оперативной памяти (напомню мы используем 4Мб). И тут есть 2 решения: а) в настройках эмулятора временно увеличить до 8Мб и всё заработает на надо или б) осуществить сборку в несколько этапов.
```
cd kernel && make
cd ../mm  && make
cd ../fs  && make
...
После успешной компиляции создаем необходимые директории и устройства на жестком диске.
```
cd /linux
mkdir dev
mkdir bin
...
183+0 records out
```
И перезагружаем компьютер. И поздравляю, вы в Linux-0.01 с [захардкоренной](http://www.oldlinux.org/Linux.old/Linux-0.01/docs/RELNOTES-0.01) [финской](https://upload.wikimedia.org/wikipedia/commons/1/1c/IBM_model_M2_for_Sweden_and_Finland.jpg) раскладкой.

![linux-0.01](https://habrastorage.org/webt/nn/yy/qp/nnyyqp3ss4bk1plc0y4_gjspn-4.png)

Но как и писал Линус делать в ней в этой версии ядра совершенно нечего, только любоваться. Голая оболочка bash в которой нет ничего, кроме встроенных команд, даже `ls` нельзя сделать, но можно попробовать её имитировать `alias ls='echo *'`.
...
# Заключение
В качестве заключения, хочется отметить, что кроме Linux 0.01 мы сделали 3 различные среды в которых можно долго и увлекательно проводить время.

Во-первых, это оригинальная Minix 1.5.10, в которой можно изучить микроядро Minix, а книга Эндрю поможет в этом. За время жизни сообщество создала и портировало немало интересных программ. Часть их них, до сих пор можно найти в интеренете. Часть из них сохранена на легендарном [nic.funet.fi](http://www.nic.funet.fi/pub/minix/).

Во-вторых, это Minix-386, для которой тоже сделали немало интересного, начиная, что была реализация GUI Tiny-X (не X11 совместимая), виртуальные консоли (прям как современном screen), реализация [IP стека](http://www.nic.funet.fi/pub/minix/communications/tnet4.tar.Z) которого не было в Minix, но очень многие ее просили и другое.

И в третьх, среду с gcc и Linux, где можно самостоятельно скомпилировать и запустить другие версии ядра Linux или попробовать портировать GNU утилиты. К сожалению, следующие версии ядра 0.02 и 0.03 не сохранились до наших дней, а [0.10](https://mirrors.edge.kernel.org/pub/linux/kernel/Historic/old-versions/tytso/linux-0.10.tar.gz) сохранилась только версия [Theodore Ts'o](https://en.wikipedia.org/wiki/Theodore_Ts%27o). А с версией 0.11/0.12 [изучать](http://www.oldlinux.org/download/ECLK-5.0-WithCover.pdf) внутрее строение ядра Linux.

# P.S.
Если у кого-нибудь будет желание запустить всё на реальном, а не виртуальном "железе" и у вас появятся вопросы, мои контакты можно найти тут в профиле Хабра или на github.com.

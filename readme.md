# Введение
Данное руководство описывает попытку повторить, на сколько это возможно, действия Линуса Торвальдса по компиляции и запуску самой первой версии ядра Linux 0.01. Причины побудивщие Линуса начать разработка ядра хорошо написаны в книге "[Ради удовольствия: Рассказ нечаянного революционера](https://ru.wikipedia.org/wiki/%D0%A0%D0%B0%D0%B4%D0%B8_%D1%83%D0%B4%D0%BE%D0%B2%D0%BE%D0%BB%D1%8C%D1%81%D1%82%D0%B2%D0%B8%D1%8F)" (далее J4F) и во множестве других источниках. Но цель данного руководства описать детальный пошаговый процесс компиляции и запуска ядра. 

Сам процесс сборки ядра, по unix'овому очень прост, достаточно выполнить команду `make` и на выходе получить готовый бинарный файл, который надо записать на дискету. Но чтобы процесс сборки прошел успешно надо сделать большую подготовку. Вот этим мы и займемся в данном руководстве. Создадим пустую виртуальную машину в эмуляторе [86box](https://86box.readthedocs.io/) с характеристиками близкими компьютера Линуса в то время, затем установим оригинальную версию операционной системы Minix 1.5.10, применим патчи "царя и бога Minix-386" (c) Брюса Эванса и затем поставим порт gcc версии 1.37.1 для Minix-386 от Алана Блэка и Ричард Тобин и в самом конце соберем и запустим непосредственно ядро Linux с bash'ем.
...
Все дистрибутивы, скриншоты, конфигурации, эту инструкцию и даже бинарный AppImage 86box (для Linux x86_64), можно найти в репозитории https://github.com/olegslavkin/linux-0.01.
...
- [9] Предположительно, что дисковод был, т.к. в то время было распространенной практикой иметь дисководы и на 3.5, и на 5.25. Косвенное доказательно наличие дисковода на 5.25 это то, что в J4F говорится, что во время установки Minix Линусу потребовалось вставить 16 дискет (*скорее всего, на самом деле только 15*) и несмотря на то, что Minix-1.5 от PH на дискетах 3.5 [издавался](http://opennet.ru/docs/FAQ/OS/minix-info.html) и у меня, я сомневаюсь, что PH оставили дистрибутив на 17 3.5 дискетах. [Японское](https://habrastorage.org/webt/3e/wd/p5/3ewdp5n7zc0onpcjmpe_rri5cjq.jpeg) издание книги уместились на 6. <a name="note-dualfdd"></a>
...
Далее необходимо распаковать архив с пустым, неразмеченным виртуальным жестким диском на 68Мб.

> Внимательные читатели спросят, почему диск 1, а не 2 как в таблице с характеристиками? И почему он совершенно с другими характеристиками? Отвечу, изначально когда начал писать данное руководство, о 2-х жестких дисках и их характеристиках ещё не знал и поэтому создал только 1 диск. Я попытался адаптировать руководство под 2 диска, но не справился с разметкой в Minix :( 
> В будущих релизах, адаптирую под 2 диска и, возможно, не буду хранить вместе и 16 и 32 битные утилиты Minix/Minix-386.
...
В Minix 1.5.10, имеется, по меньшей мере, 2 учетные записи. Первая из них - это привелигированная `root`овая учетная запись с паролем `Geheim`, а также имеется учетная запись обычного пользователя `ast` с паролем `Wachtwoord`. Авторизуйтесь как `root`.
...
Произведем форматирование раздела, а также подключим его в автомонитрование при старте Minix-386.
```
mkdir /linux
mkfs /dev/hd4 8024
mined /etc/rc

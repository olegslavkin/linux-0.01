...
```
> В данной инструкции строка уже добавлена в disk04, но если хотите сделать это самостоятельно в репозитории есть оригинальная дискета [disk04.img.orig](dist/minix/disk04.img.orig)
...
Подготавливаем загрузочную дискету. Вставляем дискету [shoelace.img](dist/minix-386/shoelace.img) и форматируем её.
```
mkfs /dev/at0 1200
```
...
/bin3/sync
```
Вставляем дискету disk02 и перезагружаемся. После перегрузки вставляем дискету с [gcc-1.37.1](dist/gcc-1.37.1-plains/awb-gcc-1.37.1.img)
...
Загружаемся с дискеты shoelace, а затем меняем её на дискету c [gcc-1.37.1](dist/gcc-1.37.1-plains/awb-gcc-1.37.1.img), монтируем ее и копируем 16-ти версию утилиты compress (*в minix использовалась [13-ти версия](https://www.unix.com/man-page/minix/1/compress/)*).
```
mount /dev/at0 /user
...
mkdir src
```
Вставляем дискету с исходными кодами Linux [linux-src-0.01](dist/linux/src.img) и монтируем дискету. Сам архив 
...
```
Вставляем дискету [bin](dist/linux/bin.img) c готовыми бинарными [bash](http://www.oldlinux.org/Linux.old/gnu/bash/bash-1.05-linux.tar.gz) и [update](http://www.oldlinux.org/Linux.old/Linux-0.01/binaries/sbin/update.Z). 
...
```
Вставляем чистую дистеку [boot](dist/linux/boot.img) и записываем на нее ядро.
```
/bin/umount /dev/at0
